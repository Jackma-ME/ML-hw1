...
def BuildNetWork(xs, ys, keep_prob):
	...
	W_conv1 = weight_variable([5,5,1,1], 'w_conv1') # patch 5x5, in size 1, out size 32
	b_conv1 = bias_variable([1], 'b_conv1')
	h_conv1 = tf.nn.relu(conv2d(x_image, W_conv1) + b_conv1) 
	h_pool1 = max_pool_2x2(h_conv1)

	W_fc1 = weight_variable([64*64*1, 10], 'w_fc1')
	b_fc1 = bias_variable([10], 'b_fc1')

	h_pool2_flat = tf.reshape(h_pool1, [-1, 64*64*1])
...
def train(data_dir):
	...
	train_img, train_label = read_TFR(data_dir, "train", 75)
...
def test(data_dir):
	...
	pre_list = []
	la_list = []
	val_img, val_label = read_TFR(data_dir, "val", 1)
	xs = tf.placeholder(tf.float32, [None, 16384]) # 128x128
	ys = tf.placeholder(tf.float32, [None, 10])
	keep_prob = tf.placeholder(tf.float32)
